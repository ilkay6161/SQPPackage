...
function create_clarabel_solver(n::Int, m::Int, settings::Settings)
    try
        ...
        problem_data = Dict{String, Any}()  # Leeres Problem-Daten-Dict
        return ClarabelSolver(nothing, solver_settings, problem_data, n, m)
    catch e
        error("Fehler beim Erstellen Clarabel-Solver: $e")  # Fehlerbehandlung
    end
end
...
function solve_qp_clarabel(qp_solver::ClarabelSolver, H, g, J, lcon, ucon, lvar, uvar, settings)
    try
        ...
        result = Clarabel.solve!(qp_solver.solver)

        if result.status == Clarabel.SOLVED
            y = extract_constraint_multipliers(result.z, length(b_eq), length(b_ineq))
            zL, zU = extract_bound_multipliers(result.z, length(b_eq) + length(b_ineq), length(b_eq))
